...
- **Lint gate cadence:** Tasks 2–5 add `pub(crate)` items that are not fully referenced by the lib target until Task 6 adds the output-framing consumers (`prompt_frame`/`encode_output` use `GA`, `EOR_CMD`, `Negotiator::eor_enabled`, `Negotiator::charset`, and the CHARSET-REJECTED handling). `rustc`'s `dead_code` warning on the lib target is expected across Tasks 2–5 (plain `cargo test` still passes — warnings are not errors there). Per-task verification for Tasks 2–5 is `cargo test -p mud-net`; the full `cargo clippy --workspace --all-targets -- -D warnings` gate runs in Task 1 (self-contained) and from **Task 6** onward, and is mandatory in Task 7. (Correction during execution: the gate was originally scheduled at Task 5, but Task 5 only wires the modules — the framing consumers land in Task 6.)
...
Do NOT run the full `cargo clippy -- -D warnings` gate here. After Task 5's
wiring, five items remain unconsumed by production because their consumers
land in Task 6: `GA`, `EOR_CMD`, `Negotiator::eor_enabled`,
`Negotiator::charset` (used by `prompt_frame`/`encode_output`), and
`CHARSET_REJECTED` (see the fix below). `rustc dead_code` on these is
expected until Task 6; the full gate runs from Task 6 onward (see Global
Constraints — Lint gate cadence).

- [ ] **Step 4b: Fix `CHARSET_REJECTED` production handling**

`on_charset_subnegotiation` in `crates/mud-net/src/telnet/negotiation.rs`
(from Task 4) only matches `CHARSET_ACCEPTED`, leaving `CHARSET_REJECTED`
referenced by tests alone — a genuine dead constant even after Task 6.
Handle rejection explicitly (rejection means stay on ASCII, the default),
which documents the protocol reaction and makes the constant a real
production reference. Replace the method body:
...
The existing Task 4 tests (`charset_accepted_switches_to_utf8`,
`charset_rejected_stays_ascii`) still pass unchanged.
