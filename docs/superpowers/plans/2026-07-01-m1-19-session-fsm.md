...
enum State {
    ...
    // Awaiting the async auth result; carries no data — the username is already
    // in the emitted effect and the result carries the resolved account.
    AwaitingAuth,
    // Fields are carried across turns and read by `puppet_select_input` (Task 5),
    // so they are legitimately write-only until then. The scoped allow is removed
    // in Task 5 when the reads land.
...
Extend `State`. This task's `puppet_select_input`/`select_puppet`/`create_puppet`
read `PuppetSelect`'s `account` and `puppets` fields, so **remove the
`#[allow(dead_code)]` attribute added above the `PuppetSelect` variant in Task 3**
— the fields are now live and the suppression is no longer needed:
...
> **Cleanup carried over from Task 7+8:** the `RegistryResolver` in this task is
> the first genuine reader of the in-world binding, so remove **both** temporary
> dead-code suppressions added there, in `crates/mud-engine/src/session/mod.rs`,
> and confirm the build stays warning-free without them:
> 1. the scoped `#[allow(dead_code)]` attribute (and its `// LINT:` comment) above
>    `pub struct InWorldBinding` — its `account`/`puppet` fields are now read by
>    the resolver;
> 2. the `_binding` placeholder in `on_input`'s `Some(SessionState::InWorld(_binding)) => Routing::InWorld`
>    arm — revert it to a bare `_` (the resolver now reads the `InWorld` payload,
>    so the arm no longer needs to pretend-read it to avoid a dead-code warning).
...
## Task 10: End-to-end integration test — login of an existing puppet

**Scope note (read first).** This test proves the full **login → resolve** seam
against a real `mud-db`-backed backend: seed an account and puppet, boot a
`PersistentWorld` that hydrates the puppet, then drive the session FSM through
`connect → login → play` and assert the **real `RegistryResolver`** resolves the
in-world session to that puppet at its persisted location. It deliberately does
**not** exercise `new <name>` (create-then-enter), because a puppet created
mid-session is not hydrated into the running `World` — that live hydration is
wired at **M1-22** (see `PLAN.md` §M1-22 and the design doc's "Gap (resolved at
M1-22)"). The create → enter FSM path is already unit-tested with a fake backend
(Task 8). Assertion is via the resolver (which takes `&World`), **not** the
command `Pipeline` — `PersistentWorld` exposes only `world(&self) -> &World`, no
`&mut World`, and resolving the in-world caller is the integration point M1-19
owns.
...
**Interfaces:**
- Consumes: `SessionService`, `LoginBackend`, `RegistryResolver`, `SessionResolver`, `builtins::register`, `mud_db::{Accounts, PersistentWorld, PlaceMap, TenantDb}`, `mud_account::Credential`, `mud_core::World::location_of`.
...
cargo add -p mud-engine --dev mud-schema --path crates/mud-schema
cargo add -p mud-engine --dev secrecy
cargo add -p mud-engine --dev tempfile
cargo add -p mud-engine --dev tokio --features "macros,rt"
```
...
Create `crates/mud-engine/tests/session_login.rs`. Model the DB/world setup on
`crates/mud-db/tests/accounts.rs` (that file is the proven harness — mirror its
`TempDir` + `TenantDb::open` + `PlaceMap::from_pairs` + `PersistentWorld::load`
setup). Two SQLite handles to the same tempdir path are used concurrently: one
owned by the loaded `PersistentWorld`, one behind an `Accounts` for the login
reads (the seeded puppet exists in both because it was committed before either
handle opened). The backend performs read-only account lookups during login (no
`create_puppet` is driven in this test).
...
use mud_account::{Account, AccountId, Credential, LoginError, Puppet, PuppetName, RegisterError, Username};
...
fn tenant() -> TenantTag { TenantTag::new(1).expect("tenant in range") }
fn hall() -> PlaceId { PlaceId::new(NonZeroU64::new(HALL).expect("non-zero place id")) }
fn hall_slug() -> PlaceKey { PlaceKey::parse("hall").expect("valid slug") }
fn places() -> PlaceMap { PlaceMap::from_pairs([(hall(), hall_slug())]) }
fn sid(n: u64) -> SessionId { SessionId::new(NonZeroU64::new(n).expect("non-zero session id")) }
...
impl LoginBackend for DbBackend<'_> {
    ...
    async fn register(
        ...
        let credential = tokio::task::spawn_blocking(move || Credential::hash(&exposed))
            ...
            .map_err(|_| BackendError)?
            .map_err(|_| BackendError)?;
        self.accounts.register(username.clone(), &credential).await.map_err(|_| BackendError)
    }
    ...
    async fn create_puppet(
        ...
        self.accounts.create_puppet(account, name.clone(), &self.start).await.map_err(|_| BackendError)
    }
...
async fn seed(dir: &TempDir) {
    ...
    accounts
        .create_puppet(account.id, PuppetName::parse("arden").expect("puppet name"), &hall_slug())
        .await
...
async fn login_of_an_existing_puppet_resolves_in_world() {
    ...
    let world = PersistentWorld::load(world_db, tenant(), places()).await.expect("load world");
    let accounts_db = TenantDb::open(dir.path()).await.expect("open accounts db");
    let backend = DbBackend { accounts: Accounts::new(&accounts_db), world: &world, start: hall_slug() };

    let mut svc = SessionService::new("WELCOME");
    svc.connect(sid(1));
    for line in ["login alice", "hunter2", "play arden"] {
        match svc.on_input(sid(1), line, &backend).await {
            Routing::Login { close, .. } => assert!(!close, "closed early on {line:?}"),
            other => panic!("expected Login routing on {line:?}, got {other:?}"),
        }
    }
    // The session is now in-world; further input routes to the pipeline.
    assert!(matches!(svc.on_input(sid(1), "look", &backend).await, Routing::InWorld));
    ...
    let resolver = svc.resolver(&builtins);
    let resolved = resolver.resolve(sid(1), world.world()).expect("in-world session resolves");
    assert_eq!(resolved.caller.location(), hall(), "puppet resolves at its persisted room");
}
...
async fn a_wrong_password_then_retry_succeeds() {
    ...
    let mut svc = SessionService::new("W");
    svc.connect(sid(1));
    svc.on_input(sid(1), "login alice", &backend).await;
    match svc.on_input(sid(1), "wrong", &backend).await {
        Routing::Login { outputs, close } => {
            assert!(!close);
            let text = outputs.iter().map(|o| o.text.as_str()).collect::<String>();
            assert!(text.contains("Login failed"), "expected non-leaky failure, got: {text}");
        }
        other => panic!("expected Login, got {other:?}"),
    }
    // Still pre-login: retry with the right password reaches the world.
    for line in ["login alice", "hunter2", "play arden"] {
        assert!(matches!(svc.on_input(sid(1), line, &backend).await, Routing::Login { close: false, .. }));
    }
    assert!(matches!(svc.on_input(sid(1), "look", &backend).await, Routing::InWorld));
}
```

> `CallerContext` exposes `.location() -> PlaceId` and `.caller() -> EntityId`
> (see `crates/mud-engine/src/caller.rs`); assert on `.location()`. If
> `SessionResolver` / `Dispatcher` are not already re-exported from
> `mud_engine`'s crate root, add them to the `pub use` in `crates/mud-engine/src/lib.rs`
> (they are public API the test consumes) — `SessionResolver` and `Dispatcher`
> are already exported (grep `lib.rs`); use whatever names it exports.

- [ ] **Step 3: Run to verify it fails, then make it pass**

Run: `cargo test -p mud-engine --test session_login`
Expected: FAIL first (missing dev-deps / any name mismatch), then PASS once the
setup compiles. Resolve any borrow-checker or API-name issue against the real
`mud-db`/`mud-account` signatures (mirror `crates/mud-db/tests/accounts.rs`);
**no `todo!` may survive into the committed test.** If two concurrent
`TenantDb::open` handles on one path cause a SQLite lock error under the test
runtime, serialize the login reads by scoping the `accounts_db`/backend, or
confirm the DB opens in WAL/multi-reader mode — but the puppet must be committed
by `seed` before either boot handle opens.
...
cargo fmt --check
jj commit -m "test(mud-engine): end-to-end existing-puppet login integration (M1-19)"
```
