...
> the first reader of `InWorldBinding`'s `account`/`puppet` fields. When it
> compiles, **remove the scoped `#[allow(dead_code)]` attribute (and its `// LINT:`
> comment) above `pub struct InWorldBinding` in `crates/mud-engine/src/session/mod.rs`**
> — the fields are now genuinely read, so the suppression is no longer needed and
> the build must stay warning-free without it.
