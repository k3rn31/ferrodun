...
## 2026-07-09 — Remove tenant `FERRODUN_` env overrides

- **Spec:** §3.11.2 (a tenant's config MUST NOT bleed into another) + CLAUDE.md YAGNI / illegal-states-unrepresentable. SPEC does not mandate any tenant env override; the `§4.1` the code cited is only the KDL requirement.
- **Done:** dropped `.merge(Env::prefixed("FERRODUN_"))` from `TenantConfig::load` so a tenant is described solely by its own `config.toml`. The env layer read the single process environment, so in a multi-tenant process a `FERRODUN_*` var applied to *every* tenant at once (a §3.11.2 bleed) while pretending to be per-tenant; the overridable fields (`start_room`, `tenant_tag`, `locale`, `banner`, `palette`) are all builder content with a natural home in the file, not secrets/env values. Removed the now-unused `Env` import and the `environment_overrides_the_file` test; rewrote the module + `load` doc comments to state the deliberate no-override design. Updated `docs/docs/building/world-files.md` (dropped the "override with `FERRODUN_`" paragraph, replaced with the file-is-sole-source-of-truth note). Mirrors the earlier L0 rename of `FERRODUN_LOG_FORMAT`→`MUDD_LOG_FORMAT` for the same process-global-can't-be-tenant-specific reason.
- **Verify:** `cargo clippy -p mud-world --all-targets` clean; `cargo test -p mud-world` 65 passed; `cargo check --workspace` clean (public `TenantConfig::load` signature unchanged, so `mudd` unaffected); `uv run mkdocs build --strict` clean.
- **Next:** none. If a genuinely environment-specific or secret tenant field ever appears, design a scoped mechanism for that field then — do not reintroduce a blanket env layer.

## 2026-07-10 — Docs refactor: consistency sweep, nav reorder, final strict build

- **Spec:** `docs/superpowers/specs/2026-07-09-docs-refactor-design.md` — final task (15) of the docs refactor: current-state-only, persona-driven docs site; no roadmap/aspirational language; consistent terminology; clean nav ordering.
